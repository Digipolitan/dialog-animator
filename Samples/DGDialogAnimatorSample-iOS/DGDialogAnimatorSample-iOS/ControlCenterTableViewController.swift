	...
	override func viewDidLoad() {
		...
		self.actions = [
			...
			IndexPath(row: 2, section: 0): {
                ...
                options.hold = false
				options.backdrop = false
				options.blurEffectStyle = .extraLight
			...
			IndexPath(row: 0, section: 1): {
				...
				DGDialogAnimator.default.animate(view: self.confirmAlertController.view, in: self.view, with: options, path: DGDialogAnimator.AnimationPath(initial: .bottom)) {
			...
			IndexPath(row: 1, section: 1): {
				...
				DGDialogAnimator.default.animate(view: self.yesNoAlertController.view, in: self.view, with: options, path: DGDialogAnimator.AnimationPath(initial: .bottom)) {
					self.tableView.isScrollEnabled = true
				}
			},
			IndexPath(row: 2, section: 1): {
				...
				DGDialogAnimator.default.animate(view: self.intputAlertController.view, in: self.view, with: options, path: DGDialogAnimator.AnimationPath(initial: .bottom)) {
					self.tableView.isScrollEnabled = true
				}
			},
			IndexPath(row: 0, section: 2): {
                ...
                DGDialogAnimator.default.animate(view: self.modalController.view, in: self.view, with: options, path: DGDialogAnimator.AnimationPath(initial: .bottom, intermediate: .bottom))
			}
		]
	}
}
